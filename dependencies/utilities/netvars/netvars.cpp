...
namespace netvar_manager {
	...
	void add_props_for_table(netvar_table_map & table_map, const uint32_t table_name_hash, const std::string & table_name, recv_table * table, const bool dump_vars, std::map< std::string, std::map< std::uint32_t, std::string > > & var_dump, const size_t child_offset = 0) {
		for (auto i = 0; i < table->props_count; ++i) {
			...
			const auto offset = std::uint32_t(prop.offset) + child_offset;
	...
	void initialize_props(netvar_table_map & table_map) {
		...
		std::map< std::string, std::map< std::uint32_t, std::string > > var_dump;
